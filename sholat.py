# sholat
# Cl4y Tutorial
# created 2/oktober/2023 00:00 pm
#-*-coding : utf-8-*-
'''
...
tapi saya benar-benar nggak rela kalo script ini di tiru tapi tidak menyantumkan sumber scriptnya (Cth=Script By Cl4y Tutorial)
jadi mohon hargai hasil karya orang
...
ya,
gw sadar ini gk sebagus punya org lain
gak rapih, asal asalan
tapi percayalah.
ini hasil usaha gw.
...
def gettime():
      ...
      try:
                        ...
                        print(lg+'\nAstaghfirullah..\nAnda lupa ngidupin jaringannya'+x)
                        input(lg+'\nEnterin aja')
                        menu()
...
ctrl + c untuk berhenti''')
            while True:
                  ...
                  if tt == s:
                        ...
                        print (lw+f'                 {lg}WAKTUNYA BANGUN SAHUR !!!{lw}\n                        untuk wilayah\n               kota {o[7]} dan sekitarnya\n\n{lg}Credit:{x} https://youtu.be/EXjt18hF6UY')
                        print (lg+'_'*63)
...
def banner():
      ...
      ╚══════╝╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝   ╚═╝
              {lw}Programmer Muslim Nggak Lupa Ibadah{lg}
                  {lg}[{x}Spesial Ramadhan 1444 H{lg}]
...
def update():
      banner()
      print(lr+'Jangan di cancell ya cuy biar nggak error :*')
      print(lg+'Cek jaringan..')
      try:
            get('https://github.com')
      except requests.exceptions.ConnectionError:
            print(lg+'Astaghfirullah.. anda lupa ngidupin jaringannya')
            exit()
      print(lg+'Mengupdate..\nLama tidaknya tergantung jaringan, sabarr :)')
      
      os.system('cd .. && rm -rf sholat')     
      sp.call(['cd .. && git clone https://github.com/BjourkaTutorial/sholat'],shell=True, stdout=sp.DEVNULL,stderr=sp.STDOUT)
...
{lg}Nama        : {lw}Sholat
{lg}Versi       : {lw}1.3 (update: 2 oktober 2022, 00:00PM)
{lg}Tanggal     : {lw}2 Oktober 2022
{lg}Author      : {lw}Mr.Cl4y
{lg}Tujuan      : {lw}Mengingatkan kita pada
              waktu sholat
{lg}Terimakasih : {lw}Allah SWT
              Muslim Cyber Army dan semua
              umat Muslim seplanet bumi.
{lg}NB          : {lw}Manusia nggak ada yang sempurna,
              sama kaya anda, gk sempurna wkwkwk canda:v
              Silahkan laporkan kritik atau saran
              ke:  - vertonix11@gmail.com''')
